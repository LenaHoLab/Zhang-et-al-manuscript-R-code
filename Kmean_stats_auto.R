...
genes_to_use=c("Mitocarta","TF","Training_sORFs")
...
### Remove common genes between candidates or peptides with TF and Mitocarta
if(length(genes_to_use==3)){
  D[[1]]=D[[1]][,!colnames(D[[1]])%in%colnames(D[[3]])]
  D[[2]]=D[[2]][,!colnames(D[[2]])%in%colnames(D[[3]])]
}

# MitoVI=c("ENSG00000109390","ENSG00000129282","ENSG00000138495","ENSG00000141682","ENSG00000161281","ENSG00000166920","ENSG00000174021",
#          "ENSG00000232859","ENSG00000240230","ENSG00000243147")
# D[[3]]=subset(D[[3]],colnames(D[[3]])%in%MitoVI)
...
Pathways_to_keep=subset(all_pathways,all_pathways%in%rownames(D[[1]][rowSums(abs(D[[1]])>3) >100,]))
Dataset=c()
...
Matrix=Matrix[,-1]
Matrix=as.matrix(merge(Matrix,D[[3]],by="row.names",all.x=TRUE))     # Choose to consider peptides
rownames(Matrix)=Matrix[,1]
Matrix=Matrix[,-1]
...
for (nb.clusters in 2:15) {
  ...
  # Calculate the kmean score per candidate
